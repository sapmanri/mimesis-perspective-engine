...
네 층의 분업 (Vase 판정 확정 요약, 2026-07-19):

```text
Foundation          질문에서 관점의 구조와 이동 가능성을 산출
Conversation Layer  지금 무엇을 물을지 결정 (또는 T0 — 묻지 않기로 결정)
Seat Context        질문의 속도·거리·머무름 정도를 제한
Expression Layer    문장 길이·호흡·표현을 구성
```

**좌석은 Foundation의 사고 결과를 바꾸지 않는다. 같은 관점을 어떤 속도와 거리로
건넬지만 정한다.** 좌석 맥락은 파이프라인 ④~⑤에만 적용되고 ②에는 입력되지 않는다.
