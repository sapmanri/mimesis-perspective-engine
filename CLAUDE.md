...
- **한 자리에는 끝이 있다** — 저녁은 주고받은 횟수로 깊어진다(`EVENING_SPOKEN=14`, 3회 남으면 알림). 빠지는 빛은 **북창 잔광**이고 벽난로는 남는다(초는 존치 확정이지만 A석에만 있어 세 자리 공통 장치가 못 된다). 다 빠지면 쫓아내지 않고 **마무리 카드로 인도**한다. **남은 횟수를 숫자로 보이지 않는다.** 엔진은 이 규칙을 모른다 — 서버는 "새 영역을 열지 말고 모으라"는 대화 층위의 말만 건넨다(창·빛·초를 한 글자도 말하지 않는다).
